...
sample_data <- data.frame(
  Raw_Name = c(" D-Glucose (2S,3R)", "Testosterone [Similar to Androgen]", "-123-Cholestane-1-ene"),
  Raw_Formula = c(" C6 H12 O6 ", "C19H28O2", " Na Cl ")
)

# 标准化名称和分子式
sample_data$Standardized_Name <- standardize_name(sample_data$Raw_Name)
sample_data$Standardized_Formula <- standardize_formula(sample_data$Raw_Formula)
```
...
# 查看匹配结果
head(mapped_data[, c("Raw_Name", "KEGG_ID", "KEGG_Name", "Similarity", "Status")])
```
